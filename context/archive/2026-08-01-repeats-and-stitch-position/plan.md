...
**Contract**: Context reads switch from `row_states[row.id]` to `row_progress[row.id]` (`state` and `stitch_position` off the object). After the toggle `<button>`, when `state == RowStateEnum in_progress` (compare via `state.value == "in_progress"`), render a `<label class="row-stitch">stitch <input type="text" name="stitch_position" inputmode="numeric" pattern="[0-9]*" value="{{ stitch_position or '' }}" hx-post=".../reps/{{ rep_number }}/rows/{{ row.id }}/stitch" hx-trigger="change" hx-target="closest li" hx-swap="outerHTML" hx-on::responseError="<same handler as toggle>" aria-label="Stitch position for row {{ row.position }}"></label>`. The input is `type="text"`, not `type="number"`: a number input blocks non-numeric typing entirely, which would make the invalid-input error path unreachable — the 1–9999 digit range is enforced by the route instead. The `name="stitch_position"` attribute is required because HTMX serializes only named fields; without it every change would post blank and silently clear the stored value. When `stitch_error` is truthy, add `row-item--error` to the `<li>`'s class list (same visual signal as a failed request). The `<li>` layout becomes a flex row (button `flex: 1`, label shrink-0) via the page's local `<style>` block.
...
- [x] 2.1 New tests pass: `uv run pytest tests/test_row_state_routes.py -v` (stitch cases) — 76f9865
- [x] 2.2 Full suite still passes: `uv run pytest` — 76f9865

#### Manual

- [x] 2.3 Stitch input appears on in-progress row, saves on change without reload, persists across page reload — 76f9865
- [x] 2.4 Stitch value survives cycling to done and back to in-progress — 76f9865
- [x] 2.5 Clearing the input removes the position (confirmed after reload) — 76f9865
- [x] 2.6 Invalid input shows error styling; stored value unchanged — 76f9865
