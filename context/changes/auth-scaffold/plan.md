...
- [ ] 4.1 `uv run python -c "from app.main import app"` imports without error

#### Manual

- [ ] 4.2 Magic link for a new email sends a real email and creates a `User` with `password_hash=NULL`
- [ ] 4.3 Clicking the link logs the user in and redirects to `/`
- [ ] 4.4 Re-clicking the consumed link shows the error page
- [ ] 4.5 Magic link for an existing password account also works
- [ ] 4.6 Requesting a second link does not invalidate the first — both independently log the user in (until each used once)

### Phase 5: Change password (recovery path)

#### Automated

- [ ] 5.1 `uv run python -c "from app.main import app"` imports without error

#### Manual

- [ ] 5.2 `/auth/change-password` while logged out redirects to `/auth/login`
- [ ] 5.3 Magic-link-only account can set a password, then log in with email + password
- [ ] 5.4 Existing password account can change password, old password stops working
